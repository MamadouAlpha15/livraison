...
class KpiLiveController extends Controller
{
    public function __invoke(Request $request)
    {
        ...
        $totalCmdMonth       = $shop->orders()->whereMonth('created_at', $now->month)->whereYear('created_at', $now->year)->count();
        $panier              = $livres > 0 ? round($caMonth / $livres) : 0;
        $tauxLiv             = $totalCmdMonth > 0 ? round(($livres / $totalCmdMonth) * 100, 1) : 0;

        /* Delta CA vs mois précédent */
        $caLastMonth = (float) $shop->orders()->whereMonth('created_at', $now->copy()->subMonth()->month)->whereYear('created_at', $now->copy()->subMonth()->year)->where('status', 'livrée')->sum('total');
        $caDelta     = $caLastMonth > 0 ? round((($caMonth - $caLastMonth) / $caLastMonth) * 100, 1) : 0;
        ...
        $caTodayDelta = $caYest > 0 ? round((($caToday - $caYest) / $caYest) * 100, 1) : 0;
        ...
        $kanban = [
            ...
            'terminees'    => $shop->orders()->whereMonth('created_at', $now->month)->where('status', 'livrée')->count(),
            'annulees'     => $shop->orders()->whereMonth('created_at', $now->month)->whereIn('status', ['annulée', 'cancelled'])->count(),
        ];

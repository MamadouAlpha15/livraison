...
.stat-ico.tot{background:rgba(139,92,246,.12);color:#7c3aed}
.stat-ico.att{background:var(--abg);color:#d97706}
.stat-ico.con{background:var(--bbg);color:#2563eb}
.stat-ico.liv{background:var(--ibg);color:#4f46e5}
.stat-ico.ok{background:var(--gbg);color:#059669}
.stat-ico.ann{background:var(--rbg);color:#dc2626}
.stat-ico.rev{background:rgba(20,184,166,.1);color:#0d9488}
...
.f-ico{width:15px;height:15px;display:flex;align-items:center;justify-content:center;flex-shrink:0;color:var(--muted)}
...
.qs-ico{width:13px;height:13px;display:flex;align-items:center;justify-content:center;flex-shrink:0}
...
.cli-sub{font-size:10.5px;color:var(--muted);display:flex;align-items:center;gap:4px;margin-top:1px}
...
.pill-ico{width:12px;height:12px;display:flex;align-items:center;justify-content:center;flex-shrink:0}

/* status badges */
.s-att{background:var(--abg);color:#92400e;border:1px solid rgba(245,158,11,.25);font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;display:inline-flex;align-items:center;gap:5px;white-space:nowrap}
.s-con{background:var(--bbg);color:#1d4ed8;border:1px solid rgba(59,130,246,.25);font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;display:inline-flex;align-items:center;gap:5px;white-space:nowrap}
.s-liv{background:var(--ibg);color:#3730a3;border:1px solid rgba(99,102,241,.25);font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;display:inline-flex;align-items:center;gap:5px;white-space:nowrap}
.s-ok{background:var(--gbg);color:#065f46;border:1px solid rgba(16,185,129,.25);font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;display:inline-flex;align-items:center;gap:5px;white-space:nowrap}
.s-ann{background:var(--rbg);color:#7f1d1d;border:1px solid rgba(239,68,68,.25);font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;display:inline-flex;align-items:center;gap:5px;white-space:nowrap}
.st-ico{width:12px;height:12px;display:flex;align-items:center;justify-content:center;flex-shrink:0}
...
@php
    ...
    'store_sm'=> '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" '.$s.'><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4Z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>',
    ...
    'bike_sm' => '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" '.$s.'><circle cx="18.5" cy="17.5" r="3.5"/><circle cx="5.5" cy="17.5" r="3.5"/><circle cx="15" cy="5" r="1"/><path d="M12 17.5V14l-3-3 4-3 2 3h2"/></svg>',
    ...
    'pin_sm'  => '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" '.$s.'><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>',
    ...
    'dollar_lg'=>'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="2" x2="12" y2="22"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>',
    ...
    'check_fl'=> '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>',
    ...
    'clock_sm'=> '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" '.$s.'><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>',
    'phone_sm'=> '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" '.$s.'><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 13a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.34 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>',
    'eye'     => '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" '.$s.'><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/></svg>',
    'target'  => '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" '.$s.'><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>',
    ];

    // Icônes SVG par statut (pour le tableau)
    $statusMap = [
        'en_attente'   => ['class'=>'s-att','svg'=>$I['clock_sm'], 'label'=>'En attente'],
        'confirmée'    => ['class'=>'s-con','svg'=>$I['check'],     'label'=>'Confirmée'],
        'en_livraison' => ['class'=>'s-liv','svg'=>$I['bike_sm'],   'label'=>'En livraison'],
        'livrée'       => ['class'=>'s-ok', 'svg'=>$I['box_sm'],    'label'=>'Livrée'],
        'annulée'      => ['class'=>'s-ann','svg'=>$I['x_sm'],      'label'=>'Annulée'],
    ];

    // Formatage devise
    $fmtCurrency = function($code) {
        return match(strtoupper($code ?? 'GNF')) {
            'GNF'  => 'GNF', 'XOF','XAF','SEN' => 'FCFA',
            'EUR'  => '€', 'USD' => '$', 'GBP' => '£',
...
<div class="mn">
    <header class="tb">
        ...
        <div style="position:relative">
            ...
            <div class="drop" id="drop">
                <a href="{{ route('profile.edit') }}" class="drop-i"><span class="drop-ico">{!! $I['user'] !!}</span>Mon profil</a>
                <a href="{{ route('admin.dashboard') }}" class="drop-i"><span class="drop-ico">{!! $I['home'] !!}</span>Dashboard</a>
                <div class="drop-sep"></div>
                <form method="POST" action="{{ route('logout') }}" style="margin:0">
                    @csrf
                    <button type="submit" class="drop-i d"><span class="drop-ico">{!! $I['logout'] !!}</span>Déconnexion</button>
                </form>
            </div>
        </div>
    </header>
    ...
    <div class="con">
        ...
        @if(session('success'))
            <div class="flash ok">{!! $I['check_fl'] !!} {{ session('success') }}</div>
        @endif
        @if(session('error'))
        ...
        <div class="ph">
            ...
            <div class="ph-acts">
                @if($filteredShop)
                    <a href="{{ route('admin.orders.index') }}" class="btn-g">{!! $I['x_sm'] !!} Voir toutes</a>
                @endif
        ...
        <div class="qs-row">
            ...
            <a href="{{ route('admin.orders.index', array_merge(request()->except('status','page'), ['status'=>'en_attente'])) }}"
               class="qs {{ $currentStatus==='en_attente' ? 'att' : '' }}">
                <span class="qs-ico">{!! $I['clock_sm'] !!}</span> En attente ({{ $stats['en_attente'] }})
            </a>
            <a href="{{ route('admin.orders.index', array_merge(request()->except('status','page'), ['status'=>'confirmée'])) }}"
               class="qs {{ $currentStatus==='confirmée' ? 'con' : '' }}">
                <span class="qs-ico">{!! $I['check'] !!}</span> Confirmées ({{ $stats['confirmee'] }})
            </a>
            <a href="{{ route('admin.orders.index', array_merge(request()->except('status','page'), ['status'=>'en_livraison'])) }}"
               class="qs {{ $currentStatus==='en_livraison' ? 'liv2' : '' }}">
                <span class="qs-ico">{!! $I['bike_sm'] !!}</span> En livraison ({{ $stats['en_livraison'] }})
            </a>
            <a href="{{ route('admin.orders.index', array_merge(request()->except('status','page'), ['status'=>'livrée'])) }}"
               class="qs {{ $currentStatus==='livrée' ? 'ok' : '' }}">
                <span class="qs-ico">{!! $I['box_sm'] !!}</span> Livrées ({{ $stats['livree'] }})
            </a>
            <a href="{{ route('admin.orders.index', array_merge(request()->except('status','page'), ['status'=>'annulée'])) }}"
               class="qs {{ $currentStatus==='annulée' ? 'ann' : '' }}">
                <span class="qs-ico">{!! $I['x_sm'] !!}</span> Annulées ({{ $stats['annulee'] }})
            </a>
        </div>
        ...
        <form method="GET" action="{{ route('admin.orders.index') }}" id="filterForm" class="filter-bar">
            ...
            <div style="display:flex;align-items:center;gap:8px">
                <span class="f-ico" style="color:var(--green)">{!! $I['store_sm'] !!}</span>
            ...
            <div style="display:flex;gap:6px">
                ...
                @if($currentStatus || $currentShop || $currentSearch)
                    <a href="{{ route('admin.orders.index') }}" class="btn-reset">{!! $I['x_sm'] !!} Tout effacer</a>
                @endif
            </div>
        </form>
        ...
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:14px;flex-wrap:wrap">
            ...
            @if($currentShop)
                <span style="display:inline-flex;align-items:center;gap:5px;padding:3px 10px;border-radius:20px;background:rgba(124,58,237,.1);color:var(--brand);font-size:11.5px;font-weight:700;border:1px solid rgba(124,58,237,.2)">
                    {!! $I['store_sm'] !!} {{ $filteredShop->name ?? 'Boutique #'.$currentShop }}
        ...
        <div class="mc">
            <div class="mc-h">
                <div class="mc-t">
                    <span class="mc-ico">{!! $I['box_lg'] !!}</span>
                    Liste des commandes
                    @if($currentStatus)
                        @php $stInfo = $statusMap[$currentStatus] ?? null; @endphp
                        <span style="display:inline-flex;align-items:center;gap:5px;font-size:11px;font-weight:700;padding:2px 9px;border-radius:20px;background:var(--abg);color:#92400e">
                            @if($stInfo)<span class="st-ico">{!! $stInfo['svg'] !!}</span> {{ $stInfo['label'] }}@else{{ $currentStatus }}@endif
                        </span>
                    @endif
                    @if($filteredShop)
                        <span style="display:inline-flex;align-items:center;gap:5px;font-size:11px;font-weight:700;padding:2px 9px;border-radius:20px;background:var(--gbg);color:#065f46">
                            {!! $I['store_sm'] !!} {{ $filteredShop->name }}
                        </span>
                    @endif
                </div>
                ...
                <table class="tbl">
                    ...
                    <tbody>
                        ...
                        @php
                            ...
                            $st         = $statusMap[$order->status] ?? ['class'=>'s-att','svg'=>$I['clock_sm'],'label'=>$order->status];
                        ...
                        <tr>
                                ...
                                <div class="cli-cell">
                                    ...
                                    <div>
                                        ...
                                        @if($order->client_phone)
                                            <div class="cli-sub">{!! $I['phone_sm'] !!} {{ $order->client_phone }}</div>
                                        @elseif($order->delivery_destination)
                                            <div class="cli-sub">{!! $I['pin_sm'] !!} {{ Str::limit($order->delivery_destination,24) }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td>
                                @if($livreur)
                                    <span style="font-size:12px;font-weight:600;color:var(--text);display:flex;align-items:center;gap:5px">
                                        {!! $I['bike_sm'] !!} {{ Str::limit($livreur,16) }}
                                    </span>
                                @else
                        ...
                        <tr>
                            <td colspan="9">
                                <div class="empty-state">
                                    <div class="empty-ico">{!! $I['box_lg'] !!}</div>

...
:root{
    ...
    --green:#10b981;--blue:#3b82f6;--purple:#7c3aed;
...
.hero::before{content:'';position:absolute;top:-40px;right:-40px;width:200px;height:200px;background:radial-gradient(circle,rgba(147,197,253,.2) 0%,transparent 70%);pointer-events:none}
...
'truck_lg'=> '<svg width="38" height="38" viewBox="0 0 24 24" fill="none" '.$sl.'><rect x="1" y="3" width="15" height="13" rx="1"/><path d="M16 8h4l3 5v3h-7V8z"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>',
'bike_sm' => '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" '.$s.'><circle cx="5.5" cy="17.5" r="3.5"/><circle cx="18.5" cy="17.5" r="3.5"/><path d="M15 6h-3l-2 5H5.5"/><path d="M9 11l3-5h5l2 6h-4.5"/></svg>',
];
    $me     = Auth::user();
...
<header class="tb">
    ...
    <div class="tb-bc">
        ...
        <span>›</span><a href="{{ route('admin.revenus-entreprises.index') }}">Revenus entreprises</a>
        <span>›</span><span style="color:var(--text);font-weight:700">{{ $company->name }}</span>
    </div>
    <div class="tb-ua">
...
<div class="pw">
    <a href="{{ route('admin.revenus-entreprises.index', ['period' => $period]) }}" class="back-btn">← Retour à toutes les entreprises</a>
    ...
    <div class="hero">
        ...
        <div>
            ...
            <div class="hero-sub">Revenus de livraison · {{ $periodLabels[$period] ?? 'Tout' }} &nbsp;·&nbsp; <span style="background:rgba(147,197,253,.25);padding:2px 9px;border-radius:10px;font-size:12px;font-weight:800;color:#bfdbfe">{{ $devise }}</span></div>
        </div>
        <div class="hero-stats">
            ...
            <div class="hs" style="min-width:150px">
                <div class="hs-v sm" style="color:#93c5fd">{{ $fmt($stats['total_frais']) }}</div>
                <div class="hs-l">Frais collectés</div>
            </div>
        </div>
    </div>
    ...
    <div class="filter-bar">
        <div class="period-tabs">
            ...
            <a href="{{ route('admin.revenus-entreprises.show', ['company' => $company->id, 'period' => $val]) }}"
    ...
    <div class="tcard">
        <div style="padding:16px 20px;border-bottom:1px solid var(--bd);display:flex;align-items:center;gap:12px">
            <div style="font-size:14px;font-weight:800">Livraisons effectuées</div>
            <span style="font-size:12px;color:var(--muted)">{{ $orders->total() }} livraison{{ $orders->total() > 1 ? 's' : '' }}</span>
        </div>
        <div class="twrap">
            <table>
                <thead>
                ...
                <tr>
                    ...
                    <td><span style="font-weight:600">{{ $order->shop?->name ?? '—' }}</span></td>
                    <td><span style="font-weight:600">{{ $order->client?->name ?? $order->client_phone ?? '—' }}</span></td>
                    <td>
                        @if($order->driver)
                        <span style="display:inline-flex;align-items:center;gap:5px;font-weight:600"><span style="display:flex;color:var(--blue)">{!! $I['bike_sm'] !!}</span>{{ $order->driver->name }}</span>
                        @else<span class="muted">—</span>@endif
                    </td>
                    <td><span class="amt amt-blue">{{ $fmt($order->delivery_fee) }}</span></td>
                    <td class="muted">{{ \Carbon\Carbon::parse($order->updated_at)->format('d/m/y H:i') }}</td>
                </tr>
                @empty
                <tr><td colspan="6">
                    <div class="empty-state">
                        <div class="empty-ico" style="display:flex;justify-content:center;opacity:.2;margin-bottom:12px"><svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="3" width="15" height="13" rx="1"/><path d="M16 8h4l3 5v3h-7V8z"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg></div>
                        <div class="empty-title">Aucune livraison</div>
                        <div class="empty-sub">Aucune livraison pour cette période.</div>
                    </div>
                </td></tr>
                @endforelse
                </tbody>

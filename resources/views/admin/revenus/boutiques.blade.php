...
/* HERO */
.hero{background:linear-gradient(135deg,#064e3b 0%,#065f46 50%,#047857 100%);border-radius:16px;padding:28px 32px;display:flex;align-items:center;gap:24px;margin-bottom:24px;position:relative;overflow:hidden}
.hero::before{content:'';position:absolute;top:-40px;right:-40px;width:220px;height:220px;background:radial-gradient(circle,rgba(52,211,153,.2) 0%,transparent 70%);pointer-events:none}
...
.hero-sub{font-size:13px;color:#6ee7b7;margin-top:3px}
...
.hs-l{font-size:10px;font-weight:700;color:#a7f3d0;text-transform:uppercase;letter-spacing:.6px;margin-top:4px}
...
.fw{font-weight:700}
.shop-name{font-weight:800;font-size:14px;color:var(--text)}
.shop-ico{width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,#7c3aed,#a78bfa);display:flex;align-items:center;justify-content:center;font-size:16px;color:#fff;font-weight:800;flex-shrink:0}
.amt{font-size:13.5px;font-weight:800}
.amt-green{color:var(--green)}
.amt-blue{color:var(--blue)}
.amt-purple{color:var(--purple)}
...
.btn-detail{padding:5px 12px;border-radius:8px;background:var(--pbg);border:1px solid rgba(124,58,237,.25);color:var(--purple);font-size:12px;font-weight:700;text-decoration:none;transition:all .15s;white-space:nowrap}
...
.bar-fill{height:100%;border-radius:4px;background:linear-gradient(90deg,#10b981,#34d399)}
...
@php
    ...
    $maxBrut = $shops->max('total_brut') ?: 1;
    $d   = $heroDevise ?? null; // devise commune ou null si mixte
    $fmtHero = fn($n) => $d
...
<header class="tb">
    ...
    <div class="tb-bc">
        ...
        <span>›</span><span style="color:var(--text);font-weight:700">Revenus boutiques</span>
    </div>
    <div class="tb-ua">
...
<div class="pw">
    ...
    <div class="hero">
        ...
        <div>
            <div class="hero-title">Revenus boutiques</div>
            <div class="hero-sub">Ce que chaque boutique a encaissé · {{ $periodLabels[$period] ?? 'Tout' }}</div>
        </div>
        <div class="hero-stats">
            <div class="hs">
                <div class="hs-v">{{ $globalStats['boutiques'] }}</div>
                <div class="hs-l">Boutiques</div>
            </div>
            <div class="hs">
            ...
            <div class="hs" style="min-width:150px">
                <div class="hs-v sm">{{ $fmtHero($globalStats['total_brut']) }}</div>
                <div class="hs-l">
                    Total brut
                    @if(!$d)<span style="font-size:9px;opacity:.7;display:block">devises mixtes</span>@endif
                </div>
            </div>
            <div class="hs" style="min-width:150px">
                <div class="hs-v sm" style="color:#34d399">{{ $fmtHero($globalStats['total_net']) }}</div>
                <div class="hs-l">Total net</div>
            </div>
        </div>
    </div>
    ...
    <div class="filter-bar">
        ...
        <div class="period-tabs">
            ...
            <a href="{{ route('admin.revenus-boutiques.index', array_merge(request()->except('period','page'), ['period'=>$val])) }}"
               class="pt {{ $period === $val ? 'on' : '' }}">{{ $lbl }}</a>
            @endforeach
        </div>

        {{-- Filtre pays --}}
        <form method="GET" action="{{ route('admin.revenus-boutiques.index') }}" style="display:flex;gap:8px;align-items:center;margin-left:auto">
            ...
            @if($country)
            <a href="{{ route('admin.revenus-boutiques.index', request()->except('country','page')) }}"
    ...
    {{-- Barre de recherche boutique (toujours visible) + badge pays si actif --}}
    <div style="background:var(--card);border:1px solid var(--bd);border-radius:12px;padding:14px 18px;margin-bottom:16px">
        <form method="GET" action="{{ route('admin.revenus-boutiques.index') }}" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
            ...
            @if($search)
            <a href="{{ route('admin.revenus-boutiques.index', request()->except('search','page')) }}"
        ...
        <div style="margin-top:10px;font-size:12.5px;color:var(--brand);font-weight:600">
            Résultats pour « {{ $search }} » · {{ $shops->total() }} boutique{{ $shops->total() > 1 ? 's' : '' }} trouvée{{ $shops->total() > 1 ? 's' : '' }}
        </div>
        @endif

        @if($country)
        <div style="display:flex;align-items:center;gap:8px;margin-top:{{ $search ? '8px' : '0' }};padding-top:{{ $search ? '10px' : '0' }};border-top:{{ $search ? '1px solid var(--bd)' : 'none' }}">
            <span style="font-size:16px">{{ $getFlag($country) }}</span>
            <span style="font-size:13px;font-weight:700;color:#4f46e5">{{ $country }}</span>
            <span style="font-size:12px;color:var(--muted)">· {{ $globalStats['boutiques'] }} boutique{{ $globalStats['boutiques'] > 1 ? 's' : '' }}</span>
            <a href="{{ route('admin.revenus-boutiques.index', request()->except('country','search','page')) }}"
    ...
    <div class="tcard">
        <div class="thead-info">
            <div style="font-size:14px;font-weight:800;color:var(--text)">Toutes les boutiques</div>
            <span style="font-size:12px;color:var(--muted)">{{ $shops->total() }} boutique{{ $shops->total() > 1 ? 's' : '' }}</span>
        </div>
        <div class="twrap">
            <table>
                <thead>
                ...
                @php
                    $brut     = (float)($shop->total_brut ?? 0);
                    $livr     = (float)($shop->total_livraison ?? 0);
                    $net      = $brut - $livr;
                    $rank     = $shops->firstItem() + $i;
                    $pct      = $maxBrut > 0 ? round(($brut / $maxBrut) * 100) : 0;
                    $devise   = $shop->currency ?: 'GNF';
                    $shopPays = $shop->country ?: null;
                    $fmtShop  = fn($n) => number_format((float)$n, 0, ',', ' ') . ' ' . $devise;
                @endphp
                <tr>
                    <td>
                    ...
                    <td>
                        <div style="display:flex;align-items:center;gap:10px">
                            <div class="shop-ico">{{ strtoupper(substr($shop->name,0,1)) }}</div>
                            <div>
                                <div class="shop-name">{{ $shop->name }}</div>
                                @if($shop->livrees_count === 0)
                    ...
                    <td>
                        ...
                        <div style="display:flex;align-items:center;gap:6px">
                            <span style="font-size:16px">{{ $getFlag($shopPays) }}</span>
                            <span style="font-size:12.5px;font-weight:700;color:var(--text)">{{ $shopPays }}</span>
                        </div>
                        @else
                        <span class="muted">—</span>
                        @endif
                        <div style="margin-top:4px">
                            <span style="background:#e0e7ff;color:#4338ca;padding:2px 8px;border-radius:10px;font-size:11.5px;font-weight:800">{{ $devise }}</span>
                        </div>
                    </td>
                    <td>
                        <span style="font-size:16px;font-weight:900;color:var(--text)">{{ number_format($shop->livrees_count ?? 0) }}</span>
                        <span class="muted"> cmd</span>
                    </td>
                    <td><span class="amt amt-blue">{{ $fmtShop($brut) }}</span></td>
                    <td><span class="amt" style="color:#f97316">{{ $fmtShop($livr) }}</span></td>
                    <td>
                        <span class="amt amt-green">{{ $fmtShop($net) }}</span>
                        @if($brut > 0)
                        @php $netPct = round(($net/$brut)*100) @endphp
                        <div class="muted" style="margin-top:2px">{{ $netPct }}% de marge</div>
                        @endif
                    </td>
                    ...
                    <td>
                        <a href="{{ route('admin.revenus-boutiques.show', array_filter(['shop' => $shop->id, 'period' => $period])) }}"
                ...
                <tr><td colspan="9">
                    <div class="empty-state">
                        ...
                        <div class="empty-title">Aucune boutique</div>
                        <div class="empty-sub">Aucune boutique ne correspond aux filtres sélectionnés.</div>
                    </div>
                </td></tr>
                @endforelse
                </tbody>
        ...
        @if($shops->hasPages())
        <div style="padding:16px 20px;border-top:1px solid var(--bd)">{{ $shops->links() }}</div>
        @endif
    </div>

...
:root{
    ...
    --purple:#7c3aed;--pbg:rgba(124,58,237,.1);
...
.hero{background:linear-gradient(135deg,#0c1445 0%,#1e3a8a 50%,#1d4ed8 100%);border-radius:16px;padding:28px 32px;display:flex;align-items:center;gap:24px;margin-bottom:24px;position:relative;overflow:hidden}
.hero::before{content:'';position:absolute;top:-40px;right:-40px;width:220px;height:220px;background:radial-gradient(circle,rgba(147,197,253,.2) 0%,transparent 70%);pointer-events:none}
...
.hero-sub{font-size:13px;color:#93c5fd;margin-top:3px}
...
.hs-l{font-size:10px;font-weight:700;color:#bfdbfe;text-transform:uppercase;letter-spacing:.6px;margin-top:4px}
...
.thead-info{padding:16px 20px;border-bottom:1px solid var(--bd);display:flex;align-items:center;gap:12px}
...
td{padding:14px 16px;border-bottom:1px solid var(--bd);font-size:13px;vertical-align:middle}
tr:last-child td{border-bottom:none}
tr:hover td{background:#fafafa}
.muted{color:var(--muted);font-size:12px}
.co-ico{width:40px;height:40px;border-radius:10px;background:linear-gradient(135deg,#1d4ed8,#3b82f6);display:flex;align-items:center;justify-content:center;font-size:18px;color:#fff;font-weight:800;flex-shrink:0}
.co-name{font-weight:800;font-size:14px;color:var(--text)}
.amt{font-size:13.5px;font-weight:800}
.amt-blue{color:var(--blue)}
.amt-green{color:var(--green)}
.rank{width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:900;flex-shrink:0}
...
.rank-3{background:linear-gradient(135deg,#a16207,#ca8a04);color:#fff}
.rank-n{background:#f1f5f9;color:var(--muted)}
.btn-detail{padding:5px 12px;border-radius:8px;background:var(--bbg);border:1px solid rgba(59,130,246,.25);color:var(--blue);font-size:12px;font-weight:700;text-decoration:none;transition:all .15s;white-space:nowrap}
.btn-detail:hover{background:var(--blue);color:#fff;border-color:var(--blue)}
.bar-wrap{width:100%;background:#f1f5f9;border-radius:4px;height:5px;margin-top:5px}
...
@php
    ...
    $maxFrais = $companies->max('total_frais') ?: 1;

    $flags = [
        'Guinée'=>'🇬🇳','Guinea'=>'🇬🇳','Sénégal'=>'🇸🇳','Senegal'=>'🇸🇳',
        'Mali'=>'🇲🇱',"Côte d'Ivoire"=>'🇨🇮',"Cote d'Ivoire"=>'🇨🇮',
        'Cameroun'=>'🇨🇲','Cameroon'=>'🇨🇲','France'=>'🇫🇷',
        'Maroc'=>'🇲🇦','Morocco'=>'🇲🇦','Burkina Faso'=>'🇧🇫',
        'Niger'=>'🇳🇪','Togo'=>'🇹🇬','Bénin'=>'🇧🇯','Benin'=>'🇧🇯','Congo'=>'🇨🇬',
    ];
    $getFlag  = fn($c) => $flags[$c] ?? '🌍';
    $d        = $heroDevise ?? null;
    $fmtHero  = fn($n) => $d
        ? number_format((float)$n, 0, ',', ' ') . ' ' . $d
        : number_format((float)$n, 0, ',', ' ') . ' GNF';
    ...
    $I = [
        ...
        'search' => '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" '.$s.'><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>',
        // Grande version hero
        'truck_lg'=> '<svg width="38" height="38" viewBox="0 0 24 24" fill="none" '.$sl.'><path d="M5 17H3a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11a2 2 0 0 1 2 2v3"/><rect width="7" height="7" x="14" y="12" rx="1"/><path d="M5 17a2 2 0 1 0 4 0 2 2 0 0 0-4 0"/><path d="M15 19a2 2 0 1 0 4 0 2 2 0 0 0-4 0"/></svg>',
        // Icône table
        'truck_ico'=> '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M5 17H3a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11a2 2 0 0 1 2 2v3"/><rect width="7" height="7" x="14" y="12" rx="1"/><path d="M5 17a2 2 0 1 0 4 0 2 2 0 0 0-4 0"/><path d="M15 19a2 2 0 1 0 4 0 2 2 0 0 0-4 0"/></svg>',
    ];
@endphp
...
<aside class="sb" id="sb">
    ...
    <nav class="sb-nav">
        ...
        <a href="{{ route('admin.revenus-entreprises.index') }}" class="sb-a on"><span class="sb-i">{!! $I['truck'] !!}</span><span>Revenus entreprises</span></a>
...
<header class="tb">
    ...
    <div class="tb-bc">
        ...
        <span>›</span><span style="color:var(--text);font-weight:700">Revenus entreprises</span>
    </div>
    <div class="tb-ua">
...
<div class="pw">
    ...
    {{-- HERO --}}
    <div class="hero">
        <div class="hero-ico" style="display:flex;align-items:center;justify-content:center;width:64px;height:64px;background:rgba(255,255,255,.12);border-radius:16px;color:#fff;filter:drop-shadow(0 4px 12px rgba(0,0,0,.3));flex-shrink:0">{!! $I['truck_lg'] !!}</div>
        <div>
            <div class="hero-title">Revenus entreprises</div>
            <div class="hero-sub">Frais de livraison collectés par société · {{ $periodLabels[$period] ?? 'Tout' }}</div>
        </div>
        <div class="hero-stats">
            <div class="hs">
                <div class="hs-v">{{ $globalStats['entreprises'] }}</div>
                <div class="hs-l">Entreprises</div>
            </div>
            <div class="hs">
                <div class="hs-v">{{ number_format($globalStats['livrees']) }}</div>
                <div class="hs-l">Livraisons</div>
            </div>
            <div class="hs" style="min-width:150px">
                <div class="hs-v sm" style="color:#93c5fd">{{ $fmtHero($globalStats['total_frais']) }}</div>
                <div class="hs-l">Total frais collectés
                    @if(!$d && $globalStats['total_frais'] > 0)
                    <span style="font-size:9px;opacity:.7;display:block">devises mixtes</span>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="filter-bar">
        <div class="period-tabs">
            ...
            <a href="{{ route('admin.revenus-entreprises.index', array_merge(request()->except('period','page'), ['period'=>$val])) }}"
               class="pt {{ $period === $val ? 'on' : '' }}">{{ $lbl }}</a>
            @endforeach
        </div>
        {{-- Filtre pays --}}
        <form method="GET" action="{{ route('admin.revenus-entreprises.index') }}" style="display:flex;gap:8px;align-items:center;margin-left:auto">
            @foreach(request()->except('country','page') as $k=>$v)
                <input type="hidden" name="{{ $k }}" value="{{ $v }}">
            @endforeach
            <select name="country" class="fi" onchange="this.form.submit()" style="min-width:180px">
            ...
            @if($country)
            <a href="{{ route('admin.revenus-entreprises.index', request()->except('country','search','page')) }}"
               style="font-size:12px;color:var(--muted);text-decoration:none;font-weight:700;padding:4px 8px;border-radius:6px;background:#f1f5f9">✕</a>
            @endif
        </form>
    </div>

    {{-- Barre recherche (toujours visible) + badge pays --}}
    <div style="background:var(--card);border:1px solid var(--bd);border-radius:12px;padding:14px 18px;margin-bottom:16px">
        <form method="GET" action="{{ route('admin.revenus-entreprises.index') }}" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
            @foreach(request()->except('search','page') as $k=>$v)
                <input type="hidden" name="{{ $k }}" value="{{ $v }}">
            @endforeach
            <div style="position:relative;flex:1;min-width:200px;max-width:420px">
                <span style="position:absolute;left:11px;top:50%;transform:translateY(-50%);pointer-events:none;color:var(--muted);display:flex">{!! $I['search'] !!}</span>
                <input type="text" name="search"
                       placeholder="Rechercher une entreprise…"
                       value="{{ $search }}"
                       style="width:100%;padding:8px 12px 8px 34px;border-radius:8px;border:1px solid var(--bd);background:var(--bg);font-size:13px;font-family:inherit;outline:none;color:var(--text)">
            </div>
            <button type="submit" style="padding:8px 18px;border-radius:8px;background:var(--brand);color:#fff;border:none;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit">
                Rechercher
            </button>
            @if($search)
            <a href="{{ route('admin.revenus-entreprises.index', request()->except('search','page')) }}"
               style="padding:8px 14px;border-radius:8px;background:#f1f5f9;border:1px solid var(--bd);color:var(--muted);font-size:13px;font-weight:600;text-decoration:none">
                ✕ Effacer
            </a>
            @endif
        </form>
        @if($search)
        <div style="margin-top:10px;font-size:12.5px;color:var(--brand);font-weight:600">
            Résultats pour « {{ $search }} » · {{ $companies->total() }} entreprise{{ $companies->total() > 1 ? 's' : '' }} trouvée{{ $companies->total() > 1 ? 's' : '' }}
        </div>
        @endif
        @if($country)
        <div style="display:flex;align-items:center;gap:8px;margin-top:{{ $search ? '10px' : '0' }};padding-top:{{ $search ? '10px' : '0' }};border-top:{{ $search ? '1px solid var(--bd)' : 'none' }}">
            <span style="font-size:16px">{{ $getFlag($country) }}</span>
            <span style="font-size:13px;font-weight:700;color:#1d4ed8">{{ $country }}</span>
            <span style="font-size:12px;color:var(--muted)">· {{ $globalStats['entreprises'] }} entreprise{{ $globalStats['entreprises'] > 1 ? 's' : '' }}</span>
            <a href="{{ route('admin.revenus-entreprises.index', request()->except('country','search','page')) }}"
               style="margin-left:auto;font-size:12px;color:var(--muted);text-decoration:none;font-weight:600;padding:3px 10px;border-radius:6px;background:#f1f5f9;border:1px solid var(--bd)">✕ Retirer</a>
        </div>
        @endif
    </div>
    ...
    <div class="tcard">
        <div class="thead-info">
            <div style="font-size:14px;font-weight:800">Toutes les entreprises</div>
            <span style="font-size:12px;color:var(--muted)">{{ $companies->total() }} entreprise{{ $companies->total() > 1 ? 's' : '' }}</span>
        </div>
        <div class="twrap">
            <table>
                <thead>
                    <tr>
                        ...
                        <th>Frais de livraison collectés</th>
                        <th>Progression</th>
                        <th>Détail</th>
                    </tr>
                </thead>
                ...
                @php
                    $frais      = (float)($company->total_frais ?? 0);
                    $rank       = $companies->firstItem() + $i;
                    $pct        = $maxFrais > 0 ? round(($frais / $maxFrais) * 100) : 0;
                    $coDevise   = $company->currency ?: 'GNF';
                    $coPays     = $company->country ?: null;
                    $fmtCo      = fn($n) => number_format((float)$n, 0, ',', ' ') . ' ' . $coDevise;
                @endphp
                <tr>
                    <td>
                        <div class="rank {{ $rank === 1 ? 'rank-1' : ($rank === 2 ? 'rank-2' : ($rank === 3 ? 'rank-3' : 'rank-n')) }}">
                            {{ $rank === 1 ? '🥇' : ($rank === 2 ? '🥈' : ($rank === 3 ? '🥉' : $rank)) }}
                        </div>
                    </td>
                    <td>
                        <div style="display:flex;align-items:center;gap:10px">
                            <div class="co-ico">{!! $I['truck_ico'] !!}</div>
                            <div>
                                <div class="co-name">{{ $company->name }}</div>
                                @if($company->livrees_count === 0)
                                <div class="muted">Aucune livraison</div>
                                @endif
                            </div>
                        </div>
                    </td>
                    <td>
                        ...
                        <div style="display:flex;align-items:center;gap:6px">
                            <span style="font-size:16px">{{ $getFlag($coPays) }}</span>
                            <span style="font-size:12.5px;font-weight:700;color:var(--text)">{{ $coPays }}</span>
                        </div>
                        @else
                        <span class="muted">—</span>
                        @endif
                        <div style="margin-top:4px">
                            <span style="background:#dbeafe;color:#1d4ed8;padding:2px 8px;border-radius:10px;font-size:11.5px;font-weight:800">{{ $coDevise }}</span>
                        </div>
                    </td>
                    <td>
                        <span style="font-size:16px;font-weight:900;color:var(--text)">{{ number_format($company->livrees_count ?? 0) }}</span>
                        <span class="muted"> livraison{{ ($company->livrees_count ?? 0) > 1 ? 's' : '' }}</span>
                    </td>
                    <td><span class="amt amt-blue">{{ $fmtCo($frais) }}</span></td>
                    <td style="min-width:100px">
                        <div class="bar-wrap">
                            <div class="bar-fill" style="width:{{ $pct }}%"></div>
                        </div>
                        <div class="muted" style="margin-top:3px">{{ $pct }}% du max</div>
                    </td>
                    <td>
                        <a href="{{ route('admin.revenus-entreprises.show', ['company' => $company->id, 'period' => $period]) }}"
                           class="btn-detail">Voir détail →</a>
                    </td>
                </tr>
                @empty
                <tr><td colspan="7">
                    <div class="empty-state">
                        <div class="empty-ico" style="display:flex;justify-content:center;opacity:.2;margin-bottom:12px"><svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M5 17H3a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11a2 2 0 0 1 2 2v3"/><rect width="7" height="7" x="14" y="12" rx="1"/><path d="M5 17a2 2 0 1 0 4 0 2 2 0 0 0-4 0"/><path d="M15 19a2 2 0 1 0 4 0 2 2 0 0 0-4 0"/></svg></div>
                        <div class="empty-title">Aucune entreprise</div>
                        <div class="empty-sub">Aucune entreprise ne correspond aux filtres sélectionnés.</div>
                    </div>
                </td></tr>
                @endforelse
                </tbody>
        ...
        <div style="padding:16px 20px;border-top:1px solid var(--bd)">{{ $companies->links() }}</div>
        @endif
    </div>

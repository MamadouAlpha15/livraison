...
.fb-inp{height:34px;border:1px solid var(--bd);border-radius:8px;padding:0 10px;font-size:12px;
    font-family:var(--font);color:var(--text);background:var(--card);outline:none;width:240px;max-width:100%;transition:border-color .15s}
...
.bdg.b{color:#1d4ed8;background:var(--bbg)}
...
.btn-upgrade{background:linear-gradient(135deg,var(--brand),var(--bdk));color:#fff}
.btn-upgrade:hover{box-shadow:0 4px 14px var(--glow)}
.btn-upgrade-biz{background:linear-gradient(135deg,#2563eb,#1d4ed8);color:#fff}
.btn-upgrade-biz:hover{box-shadow:0 4px 14px rgba(37,99,235,.3)}
.btn-downgrade{background:rgba(100,116,139,.1);color:var(--muted);border:1px solid var(--bd)}
.btn-downgrade:hover{background:rgba(100,116,139,.18);color:var(--text)}
...
@media(max-width:640px){
    ...
    .btn-sm{justify-content:center;font-size:10.5px;padding:6px 8px}
...
<aside class="sb" id="sb">
    ...
    <nav class="sb-nav">
        ...
        <a href="{{ route('admin.plans.index') }}" class="sb-a on">
...
<div class="con">
    ...
    @endif
    @if(session('error'))
        <div class="flash err">{!! $I['x'] !!} {{ session('error') }}</div>
    @endif
    @if($errors->any())
        <div class="flash err">{!! $I['x'] !!} {{ $errors->first() }}</div>
    @endif
    ...
    <div class="bc">
        ...
        <span class="bs">›</span>
        <span style="color:var(--text);font-weight:600">Paramètres système</span>
    </div>

    {{-- Page header --}}
    <div class="ph">
        <div>
            <h1>{!! $I['cog'] !!} Paramètres système — Plans</h1>
            <div class="ph-sub">Passez manuellement une boutique au plan Pro, ou une entreprise de livraison au plan Business (30 jours).</div>
        </div>
    </div>
    ...
    <div class="kpi-g">
        <div class="kpi p">
            <div class="kpi-ic p">{!! $I['store'] !!}</div>
            <div class="kpi-v">{{ $stats['shops_total'] }}</div>
            <div class="kpi-l">Boutiques total</div>
        </div>
        <div class="kpi g">
            <div class="kpi-ic g">{!! $I['bolt_sm'] !!}</div>
            <div class="kpi-v">{{ $stats['shops_pro'] }}</div>
            <div class="kpi-l">Boutiques Pro actives</div>
        </div>
        <div class="kpi b">
            <div class="kpi-ic b">{!! $I['truck'] !!}</div>
            <div class="kpi-v">{{ $stats['companies_total'] }}</div>
            <div class="kpi-l">Entreprises total</div>
        </div>
        <div class="kpi a">
            <div class="kpi-ic a">{!! $I['bolt_sm'] !!}</div>
            <div class="kpi-v">{{ $stats['companies_business'] }}</div>
            <div class="kpi-l">Entreprises Business actives</div>
        </div>
    </div>
    ...
    <div class="sc">
        <div class="sc-h">
            <div class="sc-t">
                {!! $I['store'] !!} Boutiques
                <span style="font-size:11px;font-weight:600;color:var(--muted);margin-left:4px">({{ $shops->total() }})</span>
            </div>
        </div>
        <div class="sc-b">
            <form method="GET" action="{{ route('admin.plans.index') }}" class="fb">
                <input type="text" name="shop_search" class="fb-inp" placeholder="Nom, email, téléphone…" value="{{ $shopSearch }}">
                <button type="submit" class="fb-btn">{!! $I['search'] !!} Filtrer</button>
                @if($shopSearch)
                    <a href="{{ route('admin.plans.index') }}" class="fb-btn">{!! $I['x_sm'] !!} Réinitialiser</a>
                @endif
            </form>
        </div>
        ...
        @if($shops->isEmpty())
            <div class="empty">
                <div class="empty-t">Aucune boutique trouvée</div>
                <div class="empty-s">Modifiez votre recherche.</div>
            </div>
        @else
        <div class="tbl-wrap">
            <table class="tbl">
                ...
                <tbody>
                    ...
                    @php
                        $isPro = $shop->plan === 'pro' && $shop->plan_expires_at && $shop->plan_expires_at->isFuture();
                        $pts   = explode(' ', $shop->name ?? 'B');
                    ...
                    <tr>
                        <td>
                            <div class="t-comp">
                                @if($shop->image)
                                    <img src="{{ asset('storage/'.$shop->image) }}" style="width:34px;height:34px;border-radius:9px;object-fit:cover;flex-shrink:0" alt="">
                                @else
                                    <div class="t-av">{{ $av }}</div>
                                @endif
                                <div>
                                    <div class="t-name">{{ $shop->name }}</div>
                                    @if($shop->country)<div class="t-sub">{{ $shop->country }}</div>@endif
                                </div>
                            </div>
                        </td>
                        <td>
                            @if($shop->email)<div style="font-size:11.5px">{{ $shop->email }}</div>@endif
                            @if($shop->phone)<div class="t-sub">{{ $shop->phone }}</div>@endif
                            @if(!$shop->email && !$shop->phone)<span style="color:var(--muted)">—</span>@endif
                        </td>
                        <td>
                            @if($isPro)
                                <span class="bdg p">⚡ Pro</span>
                            @else
                        ...
                        <td>
                            @if($isPro)
                                <span style="font-size:11.5px">{{ $shop->plan_expires_at->format('d/m/Y') }}</span>
                            @else
                        ...
                        <td>
                            <div class="act-btns">
                                @if($isPro)
                                    <form method="POST" action="{{ route('admin.plans.shops.update', $shop) }}" style="margin:0"
                                          onsubmit="return confirm('Repasser « {{ addslashes($shop->name) }} » au plan Gratuit ?')">
                                        @csrf @method('PUT')
                                        <input type="hidden" name="plan" value="free">
                                        <button type="submit" class="btn-sm btn-downgrade">Repasser en Gratuit</button>
                                    </form>
                                @else
                                    <form method="POST" action="{{ route('admin.plans.shops.update', $shop) }}" style="margin:0">
                                        @csrf @method('PUT')
                                        <input type="hidden" name="plan" value="pro">
                                        <button type="submit" class="btn-sm btn-upgrade">⚡ Passer en Pro</button>
                                    </form>
                                @endif
                            </div>
                        </td>
        ...
        @if($shops->hasPages())
        <div class="pag">
            <div class="pag-info">Affichage {{ $shops->firstItem() }}–{{ $shops->lastItem() }} sur {{ $shops->total() }}</div>
            {{ $shops->onEachSide(1)->appends(['company_search'=>$companySearch])->links() }}
        </div>
        @endif
        @endif
    </div>
    ...
    <div class="sc">
        <div class="sc-h">
            <div class="sc-t">
                {!! $I['truck'] !!} Entreprises de livraison
                <span style="font-size:11px;font-weight:600;color:var(--muted);margin-left:4px">({{ $companies->total() }})</span>
            </div>
        </div>
        <div class="sc-b">
            <form method="GET" action="{{ route('admin.plans.index') }}" class="fb">
                <input type="text" name="company_search" class="fb-inp" placeholder="Nom, email, téléphone…" value="{{ $companySearch }}">
                <button type="submit" class="fb-btn">{!! $I['search'] !!} Filtrer</button>
                @if($companySearch)
                    <a href="{{ route('admin.plans.index') }}" class="fb-btn">{!! $I['x_sm'] !!} Réinitialiser</a>
                @endif
        ...
        <div class="tbl-wrap">
            <table class="tbl">
                ...
                <tbody>
                    ...
                    @php
                        $isBiz = $company->plan === 'business' && $company->plan_expires_at && $company->plan_expires_at->isFuture();
                        $pts   = explode(' ', $company->name ?? 'E');
                        $av    = strtoupper(substr($pts[0],0,1)) . strtoupper(substr($pts[1] ?? '',0,1));
                    @endphp
                    <tr>
                        <td>
                            <div class="t-comp">
                                @if($company->image)
                                    <img src="{{ asset('storage/'.$company->image) }}" style="width:34px;height:34px;border-radius:9px;object-fit:cover;flex-shrink:0" alt="">
                                @else
                                    <div class="t-av">{{ $av }}</div>
                                @endif
                                <div>
                                    <div class="t-name">{{ $company->name }}</div>
                                    @if($company->country)<div class="t-sub">{{ $company->country }}</div>@endif
                                </div>
                            </div>
                        </td>
                        <td>
                            @if($company->email)<div style="font-size:11.5px">{{ $company->email }}</div>@endif
                            @if($company->phone)<div class="t-sub">{{ $company->phone }}</div>@endif
                            @if(!$company->email && !$company->phone)<span style="color:var(--muted)">—</span>@endif
                        </td>
                        <td>
                            @if($isBiz)
                                <span class="bdg b">💼 Business</span>
                            @else
                                <span class="bdg m">🆓 Gratuit</span>
                            @endif
                        </td>
                        <td>
                            @if($isBiz)
                                <span style="font-size:11.5px">{{ $company->plan_expires_at->format('d/m/Y') }}</span>
                            @else
                                <span style="color:var(--muted)">—</span>
                            @endif
                        </td>
                        <td>
                            <div class="act-btns">
                                @if($isBiz)
                                    <form method="POST" action="{{ route('admin.plans.companies.update', $company) }}" style="margin:0"
                                          onsubmit="return confirm('Repasser « {{ addslashes($company->name) }} » au plan Gratuit ?')">
                                        @csrf @method('PUT')
                                        <input type="hidden" name="plan" value="free">
                                        <button type="submit" class="btn-sm btn-downgrade">Repasser en Gratuit</button>
                                    </form>
                                @else
                                    <form method="POST" action="{{ route('admin.plans.companies.update', $company) }}" style="margin:0">
                                        ...
                                        <button type="submit" class="btn-sm btn-upgrade-biz">💼 Passer en Business</button>
                                    </form>
                                @endif
                            </div>
                        </td>
        ...
        <div class="pag">
            <div class="pag-info">Affichage {{ $companies->firstItem() }}–{{ $companies->lastItem() }} sur {{ $companies->total() }}</div>
            {{ $companies->onEachSide(1)->appends(['shop_search'=>$shopSearch])->links() }}
        </div>
        @endif
        @endif
    </div>

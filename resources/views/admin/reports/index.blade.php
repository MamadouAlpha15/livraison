{{--
    ...
    Route     : GET /boutique/reports → Admin\ReportController@index → boutique.reports.index
    Variables :
      $totalOrders, $totalRevenue, $pendingOrders, $deliveringOrders,
      $deliveredOrders, $cancelledOrders, $tauxLivraison, $panierMoyen,
      $ordersThisMonth, $revenueThisMonth, $revenueDelta,
      $chartMois, $maxRevenue, $maxOrders,
      $commissionsPending, $commissionsPaid,
      $vendors, $livreurs, $topProducts,
...
.kpi-val  { font-size: 24px; font-weight: 800; font-family: var(--mono); color: var(--text); letter-spacing: -.8px; line-height: 1; }
...
.chart-tab {
    ...
    border: 1px solid var(--border-dk); background: var(--bg); color: var(--text-2);
...
.chart-bar {
    ...
    transition: height .6s cubic-bezier(.23,1,.32,1), opacity .15s;
...
@php
    ...
    /* $fmt — abrège uniquement les vrais millions.
     * En dessous de 1M on affiche le chiffre exact formaté avec espaces
     * pour éviter que 389 500 s'affiche "390k" par arrondi. */
    ...
    <nav class="sb-nav">
        ...
        <div class="sb-group">
            ...
            <div class="sb-sub open">
                ...
                <a href="{{ route('boutique.reports.index') }}" class="sb-item active">
...
<div class="page-wrap">
    ...
    {{-- ════════════════════════════════════════════════════════════
         SECTION 1 : VUE D'ENSEMBLE DU MOIS EN COURS
         Ce que le propriétaire regarde d'abord — les chiffres du mois
    ════════════════════════════════════════════════════════════ --}}
    ...
    <div class="kpi-grid-2" style="margin-bottom:14px">
        ...
        <div class="kpi" style="--kc:#10b981">
            ...
            {{-- Devise dynamique --}}
            <div class="kpi-val">{{ $fmt($revenueThisMonth) }}</div>
    ...
    {{-- ════════════════════════════════════════════════════════════
         SECTION 2 : PIPELINE COMMANDES (tous les temps)
         Comprendre où en sont les commandes en ce moment
    ════════════════════════════════════════════════════════════ --}}
    ...
    {{-- ════════════════════════════════════════════════════════════
         SECTION 3 : PERFORMANCE (métriques de qualité)
         Pour savoir si la boutique performe bien
    ════════════════════════════════════════════════════════════ --}}
    ...
    <div class="kpi-grid-3">
        ...
        <div class="kpi" style="--kc:#10b981">
            <div class="kpi-ico">✅</div>
            <div class="kpi-lbl">Commandes livrées</div>
        ...
        <div class="kpi" style="--kc:#f59e0b">
            ...
            {{-- Devise dynamique --}}
            <div class="kpi-val">{{ $fmt($panierMoyen) }}</div>
    ...
    {{-- ════════════════════════════════════════════════════════════
         SECTION 4 : GRAPHIQUE 6 MOIS
         Visualiser la tendance long terme
    ════════════════════════════════════════════════════════════ --}}
    ...
    {{-- ════════════════════════════════════════════════════════════
         SECTION 5 : FINANCES (revenus + commissions)
    ════════════════════════════════════════════════════════════ --}}
    ...
    <div class="kpi-grid-3">
        ...
        <div class="kpi" style="--kc:#10b981">
            <div class="kpi-ico">💵</div>
            <div class="kpi-lbl">Revenu net total</div>
        ...
        <div class="kpi" style="--kc:#f59e0b">
            <div class="kpi-ico">⏳</div>
            <div class="kpi-lbl">Commissions à payer</div>
            <div class="kpi-val">{{ $fmt($commissionsPending) }}</div>
        ...
        <div class="kpi" style="--kc:#3b82f6">
            <div class="kpi-ico">✅</div>
            <div class="kpi-lbl">Commissions payées</div>
            <div class="kpi-val">{{ $fmt($commissionsPaid) }}</div>
    ...
    <div class="kpi-grid-2" style="margin-bottom:22px">
        ...
        <div class="kpi" style="--kc:#2563eb">
            <div class="kpi-ico">👤</div>
            <div class="kpi-lbl">Vendeurs actifs</div>
        ...
        <div class="kpi" style="--kc:#7c3aed">
            <div class="kpi-ico">🚴</div>
            <div class="kpi-lbl">Livreurs actifs</div>
    ...
    {{-- ════════════════════════════════════════════════════════════
         SECTION 7 : TOP PRODUITS DU MOIS
    ════════════════════════════════════════════════════════════ --}}
    ...
    <div class="top-card">
        <div class="chart-card-hd">
            ...
            <a href="{{ route('products.index') }}" style="font-size:11px;color:var(--brand);font-weight:600;text-decoration:none">
                Voir tous →
            </a>
        </div>
    ...
    {{-- ════════════════════════════════════════════════════════════
         SECTION 8 : LIENS RAPIDES vers les autres sections
    ════════════════════════════════════════════════════════════ --}}
    <div class="sec-title">🔗 Accès rapide</div>
    <div class="links-grid">
        <a href="{{ route('boutique.orders.index') }}" class="quick-link">
            <span class="quick-link-ico">📦</span>
            <span class="quick-link-lbl">Commandes</span>
            <span class="quick-link-sub">{{ $pendingOrders }} en attente</span>
        </a>
        <a href="{{ route('boutique.payments.index') }}" class="quick-link">
            <span class="quick-link-ico">💳</span>
            <span class="quick-link-lbl">Paiements</span>
            <span class="quick-link-sub">Voir les revenus</span>
        </a>
        <a href="{{ route('boutique.commissions.index') }}" class="quick-link">
            <span class="quick-link-ico">💸</span>
            <span class="quick-link-lbl">Commissions</span>
            <span class="quick-link-sub">{{ number_format($commissionsPending/1000,0) }}k {{ $devise }} à payer</span>
        </a>
        <a href="{{ route('boutique.clients.index') }}" class="quick-link">
            <span class="quick-link-ico">👥</span>
            <span class="quick-link-lbl">Clients</span>
            <span class="quick-link-sub">Voir les clients</span>
        </a>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('#chartBars .chart-bar').forEach((bar, i) => {
        setTimeout(() => {
            bar.style.transition = 'height .6s cubic-bezier(.23,1,.32,1)';
    ...
    /* ── Tabs graphique Revenus / Commandes ── */

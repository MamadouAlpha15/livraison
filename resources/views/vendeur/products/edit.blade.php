            ...
            <div class="form-card" style="margin-bottom:16px">
                ...
                <div class="preview-card-product">
                    <div class="preview-card-img" id="previewImgWrap">
                        {{-- En mode edit : afficher l'image actuelle du produit dans la preview ── --}}
                        <img id="previewImgEl"
                             src="{{ $isEdit && $product->image ? asset('storage/'.$product->image) : '' }}"
                             alt="{{ $isEdit ? $product->name : '' }}"
                             style="{{ $isEdit && $product->image ? 'display:block' : 'display:none' }}">
                        <span id="previewImgPlaceholder"
                              style="{{ $isEdit && $product->image ? 'display:none' : '' }}">🏷️</span>
                    </div>
                    <div class="preview-card-body">
                        ...
                        <div class="preview-meta" id="previewMeta">
                            @if($isEdit)
                                @if($product->stock !== null)<span class="preview-chip">📦 {{ $product->stock }} en stock</span>@endif

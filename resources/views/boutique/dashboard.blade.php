...
.btn-ghost { background: transparent; border-color: transparent; color: var(--brand-dk); padding: 5px 8px; }
...
.up   { color: #15803d; } /* contraste corrigé (#16a34a échouait à 3.3:1 sur fond blanc) */
.down { color: #dc2626; }
...
.rc-delta.flat { background: #f3f4f6; color: #475569; }
...
.rc-day-dot.today { color: var(--brand-dk); font-weight: 700; }
...
.p-muted   { background: #f3f6f4; color: #475569; }
...
.sp-fill { height: 100%; border-radius: 4px; background: var(--brand); transform-origin: left; transition: transform 1.1s cubic-bezier(.23,1,.32,1); }
...
.period-bar { width: 100%; border-radius: 3px 3px 0 0; background: var(--brand); opacity: .8; transform-origin: bottom; transition: transform .4s cubic-bezier(.23,1,.32,1), opacity .15s; cursor: pointer; position: relative; min-height: 2px; }
...
.mini-bar { width:80%; border-radius:6px 6px 0 0; background:linear-gradient(180deg,#a78bfa 0%,#7c3aed 100%); opacity:.45; min-height:3px; transform-origin:bottom; transition:transform .65s cubic-bezier(.23,1,.32,1), opacity .2s, box-shadow .2s; cursor:pointer; }
...
@media (max-width: 640px) {
    .plan-banner-badge{
        white-space:normal;flex-shrink:1;width:100%;
...
@php
    ...
    $isOnTrial       = $isPro && app(\App\Services\SubscriptionService::class)->shopIsOnTrial($shop);
        ...
        <div class="content">
            ...
            <div class="plan-banner {{ $isPro ? 'pro' : '' }}">
                ...
                <div class="plan-banner-badge {{ $isPro ? 'pro' : 'free' }}">
                    @if($isOnTrial)
                        🎁 Essai gratuit Pro — encore {{ max(0, (int) now()->diffInDays($shop->plan_expires_at, false)) }} jour{{ (int) now()->diffInDays($shop->plan_expires_at, false) > 1 ? 's' : '' }} (jusqu'au {{ $shop->plan_expires_at->format('d/m/Y') }})
                    @elseif($isPro)
...
{{-- ══ ASSISTANT D'ANALYSE DES VENTES (IA) ══
     JS dans resources/js/boutique-dashboard.js.
     CSS remis ici en ligne (le fichier externe ne s'affichait pas correctement en prod). --}}
<style>
.va-chat-fab {
    position: fixed !important; left: auto !important; top: auto !important;
    right: 20px !important; bottom: 20px !important; z-index: 850;
...
.va-chat-fab-locked { text-decoration: none; opacity: .88; }
.va-chat-fab-locked:hover { opacity: 1; }
.va-chat-lock {
...
@media (max-width: 640px) {
    .va-chat-fab { bottom: 20px !important; right: 14px !important; left: auto !important; width: 50px; height: 50px; font-size: 21px; }
    .va-chat-panel {
...
{{-- Assistant IA : réservé au Plan Pro (comme les autres fonctionnalités Pro) --}}
@if($isPro)
<button class="va-chat-fab" id="vaChatFab" onclick="toggleVaChat()" title="Assistant de ventes">📊</button>
@else
<a href="{{ route('boutique.subscription.upgrade') }}" class="va-chat-fab va-chat-fab-locked" title="Assistant de ventes — réservé au Plan Pro">
    📊<span class="va-chat-lock">🔒</span>
</a>
@endif
...
function toggleVaChat() {
    ...
    if (_vaChatOpen) {
        document.getElementById('vaChatInput')?.focus();
    } else {
...
window.BQ_CFG = {
    ...
    assistantChatUrl: @json(route('boutique.assistant.chat')),
    assistantResetUrl: @json(route('boutique.assistant.reset')),
};
</script>

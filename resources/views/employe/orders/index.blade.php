{{--
    ...
    Route   : GET /employe/orders  → Employe\OrderController@index
    Route   : GET /boutique/orders → Employe\OrderController@index (boutique.orders.index)
    Variables :
      $orders   → LengthAwarePaginator<Order>
      $livreurs → Collection<User>  (livreurs de la boutique)
      $shop     → Shop
...
.sidebar {
    ...
    width: var(--sb-w);
    /* Scroll sur la sidebar ENTIÈRE — scrollbar verte 4px toujours visible */
    overflow-y: scroll;
...
.sb-nav {
    padding: 10px 10px 32px; /* padding-bottom → Support visible au-dessus du footer */
    flex: 1;
    display: flex; flex-direction: column; gap: 1px;
    /* Pas de scroll interne — c'est .sidebar qui scrolle */
    overflow: visible;
}
...
.sb-badge {
    ...
    font-family: var(--mono); min-width: 20px; text-align: center;
}
...
.sb-group-toggle .sb-arrow {
    ...
    color: rgba(255,255,255,.25); transition: transform .2s;
    flex-shrink: 0;
}
...
.sb-footer {
    ...
    /* Sticky en bas — toujours visible, fond sombre masque le contenu derrière */
    position: sticky;
    bottom: 0;
    background: var(--sb-bg);
    z-index: 1;
...
.sb-scroll-hint {
    ...
    margin-top: -40px; /* overlap avec le contenu */
    align-self: flex-end;
}
...
/* Page header */
.page-hd { display: flex; align-items: flex-start; justify-content: space-between; gap: 14px; margin-bottom: 18px; flex-wrap: wrap; }
...
@media (max-width: 900px) {
    ...
    .stats-row { gap: 6px; }
    .stat-chip { padding: 8px 10px; font-size: 11px; }
...
@php
    ...
    /* Statuts où l'annulation est possible */
    $annulables   = ['pending', 'en attente', 'en_attente', 'confirmée', 'confirmed'];
    /* Statuts où la restauration est possible */
    ...
    /* CA page — uniquement les commandes LIVRÉES (cohérent avec le dashboard) */
...
<div class="modal-overlay" id="cancelModal">
    <div class="modal-box">
        ...
        <div class="modal-sub">
            Commande <strong id="modalOrderId"></strong> — client <strong id="modalClientName"></strong><br>
            Cette action est <strong>irréversible</strong>. La commande passera au statut <strong>Annulée</strong>.
        </div>
...
<div class="modal-overlay" id="restoreModal">
    <div class="modal-box">
        ...
        <div class="modal-sub">
            Commande <strong id="modalRestoreOrderId"></strong> — client <strong id="modalRestoreClientName"></strong><br>
            La commande repassera au statut <strong>En attente</strong>.
        </div>
        <div class="modal-actions">
...
<div class="dash-wrap">
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sb-nav">
            ...
            {{-- ════════════════════════════════
                 LIVRAISON — livreurs + partenaires
            ════════════════════════════════ --}}
            ...
            {{-- ════════════════════════════════
                 FINANCES — groupe accordéon
                 Paiements · Commissions · Rapports
            ════════════════════════════════ --}}
    ...
    <main class="main">
        ...
        <div class="topbar">
            ...
            <div class="tb-info">
                <div class="tb-title">📦 Commandes à assigner</div>
                <div class="tb-sub">
                    {{ $shop->name ?? 'Boutique' }}
                    &nbsp;·&nbsp;
                    <span class="devise-badge" style="font-size:10px;padding:2px 8px">💱 {{ $devise }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="content">
            ...
            <div class="stats-row">
                <div class="stat-chip">
                    ...
                    <span class="val">{{ $nonAssignees }}</span>
                </div>
                <div class="stat-chip">
                    <span class="stat-dot" style="background:var(--brand)"></span>
                    <span>Assignées</span>
                    <span class="val">{{ $assignees }}</span>
                </div>
                <div class="stat-chip">
                    <span class="stat-dot" style="background:#3b82f6"></span>
                    <span>CA page</span>
                    <span class="val">{{ number_format($caPage, 0, ',', ' ') }} <small style="font-size:9px;font-weight:600;color:var(--muted)">{{ $devise }}</small></span>
                </div>
                <div class="stat-chip">
                    <span>Total</span>
                    <span class="val">{{ $orders->total() }}</span>
                </div>
            </div>
            ...
            <div class="orders-card desktop-table">
                <div class="orders-card-hd">
                    ...
                    <span style="font-size:11px;color:var(--muted)">
                        Page {{ $orders->currentPage() }}/{{ $orders->lastPage() }} · {{ $orders->total() }} commande(s)
                    </span>
                </div>
                ...
                <table class="tbl">
                    <thead>
                        <tr>
                            <th>#</th><th>Client</th><th>Produit</th><th>Montant</th>
                            <th>Statut</th><th>Livreur</th><th>Assigner</th><th>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div style="display:flex;align-items:center;gap:9px">
                                    ...
                                    <div>
                                        ...
                                        @if($client?->phone)<div class="c-sub">📞 {{ $client->phone }}</div>@endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td>
                                ...
                                <div style="display:flex;align-items:center;gap:8px">
                                    ...
                                    @endif
                                    <div>
                                        <div style="font-size:12.5px;font-weight:600;color:var(--text)">{{ Str::limit($product->name, 22) }}</div>
                                        <div style="font-size:11px;color:var(--muted)">Qté : {{ $order->items->first()->quantity ?? 1 }}</div>
                                    </div>
                                </div>
                                @else
                            ...
                            <td>
                                @if($order->livreur)
                                <div class="lv-chip">
                                    <div class="lv-chip-av">{{ initiales($order->livreur->name) }}</div>
                                    {{ Str::limit($order->livreur->name, 14) }}
                                </div>
                                @else
                            ...
                            <td>
                                @if(!$order->livreur)
                                <form action="{{ route('employe.orders.assign', $order) }}" method="POST" class="assign-form">
                                    @csrf @method('PUT')
                            ...
                            <td>
                                <div style="display:flex;align-items:center;gap:5px;flex-wrap:nowrap">
                                    ...
                                    <button type="button" class="btn-cancel"
                                        onclick="openCancelModal('{{ route('employe.orders.cancel', $order) }}','#{{ $order->id }}','{{ addslashes($client->name ?? 'Inconnu') }}')">
                                        ✕ Annuler
                                    </button>
                                    @elseif($dejaAnnulee)
                                    <button type="button" class="btn-restore"
                                        onclick="openRestoreModal('{{ route('employe.orders.restore', $order) }}','#{{ $order->id }}','{{ addslashes($client->name ?? 'Inconnu') }}')">
                                        🔄 Restaurer
                                    </button>
                                    @else
                                    <button type="button" class="btn-cancel disabled" disabled title="{{ $st['label'] }}">✕</button>
                                    @endif
                                </div>
                            </td>
                        </tr>
                ...
                <div class="m-card">
                    <div class="m-card-hd">
                        ...
                        <div style="display:flex;align-items:center;gap:6px;flex-shrink:0">
                            ...
                            <span style="font-family:var(--mono);font-size:10px;color:var(--muted)">#{{ $order->id }}</span>
                        </div>
                    </div>
                    ...
                    <div class="m-card-body">
                        ...
                        <div class="m-row">
                            ...
                            <span class="amount">{{ number_format($order->total, 0, ',', ' ') }} <small>{{ $devise }}</small></span>
                        </div>

                        <div class="m-row">
                            <span class="m-lbl">Livreur</span>
                            @if($order->livreur)
                            <div class="lv-chip">
                                <div class="lv-chip-av">{{ initiales($order->livreur->name) }}</div>
                                {{ $order->livreur->name }}
                            ...
                            @else
                            <span class="pill p-warning">Non assigné</span>
                            @endif
                        </div>

                        @if(!$order->livreur && $livreurs->isNotEmpty())
                        <form action="{{ route('employe.orders.assign', $order) }}" method="POST" style="display:flex;gap:8px;align-items:center">
                            @csrf @method('PUT')
                            <select name="livreur_id" class="assign-select" style="flex:1" required>
                    ...
                    <div class="m-card-foot">
                        ...
                        @endif
                        @if($peutAnnuler)
                        <button type="button" class="btn-cancel" style="flex:1;justify-content:center"
                            onclick="openCancelModal('{{ route('employe.orders.cancel', $order) }}','#{{ $order->id }}','{{ addslashes($client->name ?? 'Inconnu') }}')">
                            ✕ Annuler
                        </button>
                        @elseif($dejaAnnulee)
                        <button type="button" class="btn-restore" style="flex:1;justify-content:center"
                            onclick="openRestoreModal('{{ route('employe.orders.restore', $order) }}','#{{ $order->id }}','{{ addslashes($client->name ?? 'Inconnu') }}')">
                            🔄 Restaurer
                        </button>
                        @endif
...
/* ═══════════════════════════════════════════════════════
   ...
   - Après ouverture, scroll automatique vers Support
     pour qu'il ne soit jamais caché derrière le footer
   ═══════════════════════════════════════════════════════ */
function toggleGroup(btn) {
    ...
    if (!isOpen) {
        ...
        /* Scroll la sidebar vers Support pour qu'il reste visible */
